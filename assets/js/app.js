	...
	$(document).ready(function ($) {
		...
		if ($(".main-wrapper").length > 0) {
			...
			$("#mobile_btn").click(function () {
				...
				$(".dropdown.open > .dropdown-toggle").dropdown(
					"toggle"
				);
				return false;
			});
		...
		// 		var $element = $(this);
		// 		var $uniques = $(modalUniqueClass + ":visible").not(
		// 			$(this)
		// 		);
		// 		if ($uniques.length) {
		// 			$uniques.modal("hide");
		...
		if ($(".slimscroll").length > 0) {
			...
			var hei =
				$(window).height() - HEIGHT_HEADER - HEIGHT_FOOTER;
		...
		if ($("#drop-zone").length > 0) {
			...
			uploadForm.addEventListener("submit", function (e) {
				var uploadFiles = document.getElementById(
					"js-upload-files"
				).files;
				e.preventDefault();
		...
		if (
			$(
				"#areaChart, #bar-example, #donutChart, #area-chart"
			).length > 0
		) {
			var colors = [
				"#E94B3B",
